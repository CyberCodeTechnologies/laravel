    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="padding: 30px 0;">
            ...
            <div style="background: #f5f5f5; padding: 20px; margin: 20px 0; border-radius: 5px;">
                ...
                <p><strong>Order Number:</strong> {{ $order->formatted_order_number }}</p>
                ...
                <p><strong>Shipping Method:</strong> {{ ucfirst($order->shipping_method ?? 'standard') }}</p>
            </div>
                ...
                @if($order->items->count() > 0)
                    @foreach($order->items as $item)
                        <div style="padding: 10px 0; border-bottom: 1px solid #ddd;">
                            <p style="margin: 0;"><strong>{{ $item->artwork_title }}</strong></p>
                            <p style="margin: 5px 0; color: #666;">Quantity: {{ $item->quantity }} × {{ $item->formatted_price }}</p>
                            <p style="margin: 5px 0; font-weight: bold;">{{ $item->formatted_subtotal }}</p>
                        </div>
                    @endforeach
                @endif
            </div>
        ...
        <div style="text-align: center; padding: 20px 0; border-top: 1px solid #ddd; margin-top: 30px; color: #666;">
            <p style="margin: 0;">© {{ date('Y') }} Panchi Gallery. All rights reserved.</p>
            <p style="margin: 10px 0 0;">
                <a href="{{ route('home') }}" style="color: #000;">Visit Our Gallery</a>
            </p>
        </div>
    </div>
</body>

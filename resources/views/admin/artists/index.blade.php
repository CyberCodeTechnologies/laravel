...
@section('meta-description', 'Manage and approve artists on Panchi Gallery platform')

@section('header', 'Manage Artists')
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-blue-900">{{ App\Models\User::where('role', 'artist')->count() }}</p>
                        <p class="text-xs text-blue-700 mt-1">
                            @php
                                $newArtists = App\Models\User::where('role', 'artist')->where('created_at', '>=', now()->subDays(7))->count();
                            @endphp
                            +{{ $newArtists }} this week
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-green-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-green-900">{{ App\Models\User::where('role', 'artist')->where('is_approved', true)->count() }}</p>
            ...
            <div class="bg-yellow-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-yellow-900">{{ App\Models\User::where('role', 'artist')->where('is_approved', false)->count() }}</p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <form method="GET" class="flex flex-wrap gap-3 items-center">
                ...
                <a href="{{ route('admin.artists') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition">
                    <i class="fas fa-times mr-2"></i>Clear
                </a>
            </form>
            <div class="flex gap-3">
                <a href="{{ route('admin.pending-artists') }}" class="bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 transition">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($artists as $artist)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            @if($artist->avatar)
                                                <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/' . $artist->avatar) }}" alt="{{ $artist->name }}">
                                            @else
                                                <div class="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center">
                                                    <span class="text-gray-600 font-medium">{{ substr($artist->name, 0, 1) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $artist->artworks()->count() }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('public.artists.show', $artist->slug ?? $artist->id) }}" class="text-blue-600 hover:text-blue-900" title="View Profile">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.users.edit', $artist->id) }}" class="text-indigo-600 hover:text-indigo-900" title="Edit">
                                        ...
                                        @endif
                                        <form action="{{ route('admin.artists.delete', $artist->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')

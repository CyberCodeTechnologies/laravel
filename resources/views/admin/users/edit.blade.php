...
@section('title', 'Edit User - Admin - Panchi Gallery')
    ...
    <div class="bg-white rounded-lg shadow-sm">
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6 space-y-8">
                        ...
                        <div>
                            ...
                            <div class="flex items-start space-x-4">
                                <div class="flex-shrink-0" id="avatar-preview-container">
                                    @if($user->avatar)
                                        <img class="h-24 w-24 rounded-full object-cover border-2 border-gray-200" src="{{ asset('storage/' . $user->avatar) }}" alt="{{ $user->name }}">
                                    @else
                                        <div class="h-24 w-24 rounded-full bg-gray-200 flex items-center justify-center border-2 border-gray-300">
                                            <span class="text-gray-500 font-medium text-xl">{{ substr($user->name, 0, 1) }}</span>
                                        </div>
                                    @endif
                                </div>
                                <div class="flex-1">
                        ...
                        <div>
                            ...
                            <div class="flex items-start space-x-4">
                                <div class="flex-shrink-0" id="cover-preview-container">
                                    @if($user->cover_image)
                                        <img class="h-24 w-32 rounded-lg object-cover border-2 border-gray-200" src="{{ asset('storage/' . $user->cover_image) }}" alt="{{ $user->name }} cover">
                                    @else
                ...
                <div id="artist-section" class="{{ $user->role === 'artist' ? '' : 'hidden' }} border-b pb-6">
                    <h2 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" 
                                       id="years_active" 
                                       name="years_active" 
                                       value="{{ old('years_active', $user->years_active) }}"
                                       min="0"
                                       max="100"
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                @error('years_active')
                            ...
                            <div class="flex items-center pt-6">
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox"
                                           ...
                                           value="1"
                                           {{ old('participate_in_orders', $user->participate_in_orders ?? true) ? 'checked' : '' }}
                                           class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <textarea id="bio"
                                      ...
                                      rows="4"
                                      maxlength="1000"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('bio', $user->bio) }}</textarea>
                            <p class="text-xs text-gray-500 mt-1"><span id="bio-char-count">{{ strlen($user->bio ?? '') }}</span>/1000 characters</p>
                            @error('bio')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="artist_statement"
                                      ...
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('artist_statement', $user->artist_statement) }}</textarea>
                            <p class="text-xs text-gray-500 mt-1"><span id="statement-char-count">{{ strlen($user->artist_statement ?? '') }}</span>/2000 characters</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            const roleChanged = roleSelect && roleSelect.value !== '{{ $user->role }}';

...
@section('title', 'Support Contacts - Admin')
@section('meta-description', 'Manage customer support inquiries on Panchi Gallery')

@section('header', 'Support Contacts')
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-blue-900">{{ App\Models\ContactMessage::count() }}</p>
            ...
            <div class="bg-yellow-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-yellow-900">{{ App\Models\ContactMessage::where('status', 'pending')->count() }}</p>
            ...
            <div class="bg-green-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-green-900">{{ App\Models\ContactMessage::where('status', 'responded')->count() }}</p>
            ...
            <div class="bg-purple-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-purple-900">{{ App\Models\ContactMessage::where('status', 'resolved')->count() }}</p>
            ...
            <div class="p-6 border-b">
                <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                    <div class="flex flex-col md:flex-row gap-4 flex-1">
                        ...
                        <!-- Status Filter -->
                        <select name="status" 
                                onchange="window.location.href='{{ route('admin.support.contacts') }}?status='+this.value+'&search={{ request('search', '') }}"
                        ...
                        <input type="date" 
                               ...
                               onchange="window.location.href='{{ route('admin.support.contacts') }}?date_filter='+this.value+'&status={{ request('status', '') }}&search={{ request('search', '') }}"
                               class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                        ...
                        @forelse($contacts as $contact)
                            <tr class="hover:bg-gray-50 {{ $contact->is_read ? '' : 'bg-blue-50' }}">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <input type="checkbox" value="{{ $contact->id }}" class="contact-checkbox rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <img class="h-10 w-10 rounded-full" src="{{ $contact->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($contact->name) . '&background=random&size=128' }}" alt="{{ $contact->name }}">
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 font-medium truncate max-w-xs">{{ $contact->subject }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        ...
                                        <button onclick="respondToContact({{ $contact->id }})" class="text-green-600 hover:text-green-900">
            ...
            @if($contacts->hasPages())
                <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6 lg:px-8">
                    <div class="flex-1 flex justify-between sm:hidden">
                        ...
                        <a href="{{ $contacts->nextPageUrl() }}" class="relative ml-3 inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                            Next
                        </a>
                    </div>
...
<script>
const contactShowUrl = @json(route('admin.support.contacts.show', ['contact' => '__ID__']));
...
function markAllAsRead() {
    ...
    fetch(@json(route('admin.support.contacts.mark-read')), {
        method: 'POST',
        headers: {
...
function exportContacts() {
    window.location.href = '{{ url("admin/support/contacts/export") }}';
}

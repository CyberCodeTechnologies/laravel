...
@section('title', 'Manage Commissions - Admin')
@section('meta-description', 'Manage artist commissions and platform fees on Panchi Gallery platform')

@section('header', 'Manage Commissions')
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-blue-900">{{ App\Models\Commission::count() }}</p>
                        <p class="text-xs text-blue-700 mt-1">
                            @php
                                $totalCommissionAmount = App\Models\Commission::sum('platform_fee');
                            @endphp
                            ${{ number_format($totalCommissionAmount, 2) }} total
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-yellow-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-yellow-900">{{ App\Models\Commission::where('status', 'pending')->count() }}</p>
            ...
            <div class="bg-green-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-green-900">{{ App\Models\Commission::where('status', 'paid')->count() }}</p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <form method="GET" class="flex flex-wrap gap-3 items-center">
                ...
                <a href="{{ route('admin.commissions.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition">
                        ...
                        @forelse($commissions as $commission)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">#{{ $commission->id }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $commission->artist->name }}</div>
                                    <div class="text-sm text-gray-500">{{ $commission->artist->email }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">${{ number_format($commission->sale_amount, 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-blue-600">${{ number_format($commission->platform_fee, 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-green-600">${{ number_format($commission->artist_earnings, 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500">{{ number_format($commission->platform_fee_percentage, 2) }}%</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                                        {{ $commission->status === 'paid' ? 'bg-green-100 text-green-800' : '' }}
                                        {{ $commission->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                        {{ $commission->status === 'refunded' ? 'bg-red-100 text-red-800' : '' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('admin.commissions.show', $commission->id) }}" class="text-blue-600 hover:text-blue-900" title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if($commission->status === 'pending')
                                            <form method="POST" action="{{ route('admin.commissions.mark-paid', $commission->id) }}" class="inline">
                                                @csrf

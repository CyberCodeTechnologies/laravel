...
@section('title', 'Marketplace Management - Admin')
@section('meta-description', 'Manage secondary market listings and resales on Panchi Gallery')
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-blue-900">{{ App\Models\Resale::count() }}</p>
                        <p class="text-xs text-blue-700 mt-1">
                            @php
                                $newResales = App\Models\Resale::where('created_at', '>=', now()->subDays(7))->count();
                            @endphp
                            +{{ $newResales }} this week
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-green-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-green-900">{{ App\Models\Resale::where('status', 'listed')->count() }}</p>
            ...
            <div class="bg-yellow-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-yellow-900">{{ App\Models\Resale::where('status', 'pending')->count() }}</p>
            ...
            <div class="bg-purple-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-purple-900">{{ App\Models\Resale::where('status', 'sold')->count() }}</p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <form method="GET" class="flex flex-wrap gap-3 items-center">
                ...
                <a href="{{ route('admin.marketplace') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition">
                    <i class="fas fa-times mr-2"></i>Clear
                </a>
            </form>
            <div class="flex gap-3">
                <a href="{{ route('admin.marketplace.listings') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
                    <i class="fas fa-list mr-2"></i>All Listings
                </a>
                <a href="{{ route('admin.marketplace.pending') }}" class="bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 transition">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <img class="h-12 w-12 rounded-lg object-cover mr-3" 
                                             src="{{ $resale->artwork->primary_image ?? asset('images/placeholder-artwork.jpg') }}" 
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <img class="h-8 w-8 rounded-full object-cover mr-2" 
                                             src="{{ $resale->owner->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($resale->owner->name) . '&background=random&size=128' }}" 
                                             alt="{{ $resale->owner->name }}">
                                        <div class="text-sm text-gray-900">{{ $resale->owner->name }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ${{ number_format($resale->artwork->price, 2) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    ${{ number_format($resale->price, 2) }}
                                    @if($resale->price > $resale->artwork->price)
                                        <span class="text-green-600 text-xs">+{{ round((($resale->price - $resale->artwork->price) / $resale->artwork->price) * 100, 1) }}%</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        ...
                                        {{ $resale->status === 'sold' ? 'bg-purple-100 text-purple-800' : '' }}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('marketplace.show', $resale->artwork->slug) }}" 
                                           class="text-blue-600 hover:text-blue-900" title="View Listing">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if($resale->status === 'pending')
                                            <form method="POST" action="{{ route('admin.marketplace.approve', $resale) }}" class="inline">
                                                @csrf

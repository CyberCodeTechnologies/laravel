...
@section('title', 'Backup Settings - Admin')
@section('meta-description', 'Configure backup settings on Panchi Gallery')
            ...
            <div class="bg-indigo-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-xs text-indigo-700 mt-1">{{ now()->format('H:i') }}</p>
                    </div>
                </div>
            </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
                <form method="POST" action="{{ route('settings.backup.create') }}" class="bg-white rounded-lg shadow-md mb-6">
                    @csrf
                    <div class="p-6">
                ...
                <div class="bg-white rounded-lg shadow-md">
                    <div class="p-6">
                        ...
                        <form method="POST" action="{{ route('settings.backup.update') }}">
                            @csrf
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select id="backup_frequency" name="backup_frequency"
                                        ...
                                        <option value="manual" {{ $backupFrequency === 'manual' ? 'selected' : '' }}>Manual Only</option>
                                    </select>
                                </div>
                                <div>
                                    <label for="backup_retention" class="block text-sm font-medium text-gray-700 mb-2">Retention Period (Days)</label>
                                    <input type="number" id="backup_retention" name="backup_retention" value="{{ $backupRetention }}" min="1" max="365"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                </div>

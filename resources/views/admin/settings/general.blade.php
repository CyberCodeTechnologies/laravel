...
<section class="py-8 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('admin.settings.general.update') }}" enctype="multipart/form-data" class="space-y-6" id="settingsForm">
            @csrf
            @method('PUT')
            ...
            @foreach($groupedSettings as $group => $settings)
                <div class="bg-white rounded-lg shadow-md">
                    <div class="p-6 border-b flex justify-between items-center">
                        <h2 class="text-xl font-semibold capitalize">{{ str_replace('_', ' ', $group) }}</h2>
                        @if($group === 'branding')
                        ...
                        @foreach($settings as $setting)
                            <div class="border-b pb-6">
                                @if($setting->type === 'image')
                                    <div class="space-y-3">
                                        <div class="flex justify-between items-center">
                                            ...
                                            <div class="image-sync-status" id="sync-status-{{ $setting->key }}">
                                                <i class="fas fa-circle-notch fa-spin"></i>
                                                <span>Checking...</span>
                                            </div>
                                        </div>

                                        @if($setting->value && $setting->image_url)
                                            <div class="image-preview-container">
                                                <img src="{{ $setting->image_url }}" class="rounded border bg-gray-100" alt="{{ $setting->label }}">
                                            </div>
                                        @endif

                                        <input type="file" name="settings[{{ $setting->key }}][file]" accept="image/*" class="w-full" onchange="previewImage(this, '{{ $setting->key }}')">
                                        ...
                                        <div id="optimization-{{ $setting->key }}" class="optimization-suggestions"></div>
                                ...
                                @elseif($setting->type === 'select')
                                    <label class="block text-sm font-medium mb-2">{{ $setting->label ?? $setting->key }}</label>
                                    @php
                                    ...
                                    @endphp
                                    <select name="settings[{{ $setting->key }}][value]" class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                ...
                                @elseif($setting->type === 'json')
                                    <label class="block text-sm font-medium mb-2">{{ $setting->label ?? $setting->key }}</label>
                                    <textarea name="settings[{{ $setting->key }}][value]" rows="4" class="w-full px-3 py-2 border rounded font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder='["value1", "value2"] or {"key": "value"}'>{{ $setting->value }}</textarea>
                                    <input type="hidden" name="settings[{{ $setting->key }}][key]" value="{{ $setting->key }}">
                                    @if($setting->description)
                                        <p class="text-xs text-gray-500 mt-1">{{ $setting->description }}</p>
                                    @endif
                                ...
                                @elseif($setting->type === 'textarea')
                                    <label class="block text-sm font-medium mb-2">{{ $setting->label ?? $setting->key }}</label>
                                    <textarea name="settings[{{ $setting->key }}][value]" rows="4" class="w-full px-3 py-2 border rounded">{{ $setting->value }}</textarea>
                                    <input type="hidden" name="settings[{{ $setting->key }}][key]" value="{{ $setting->key }}">
                                    @if($setting->description)
                                        <p class="text-xs text-gray-500 mt-1">{{ $setting->description }}</p>
                                    @endif
                                @elseif($setting->type === 'number')
                                    <label class="block text-sm font-medium mb-2">{{ $setting->label ?? $setting->key }}</label>
                                    <input type="number" name="settings[{{ $setting->key }}][value]" value="{{ $setting->value }}" class="w-full px-3 py-2 border rounded">
                                    <input type="hidden" name="settings[{{ $setting->key }}][key]" value="{{ $setting->key }}">
                                    @if($setting->description)
                                        <p class="text-xs text-gray-500 mt-1">{{ $setting->description }}</p>
                                    @endif
                                @elseif($setting->type === 'boolean')
                                    <div class="flex items-center justify-between">
                                        <div>
                                            <label class="text-sm font-medium">{{ $setting->label ?? $setting->key }}</label>
                                            @if($setting->description)
                                                <p class="text-xs text-gray-500">{{ $setting->description }}</p>
                                            @endif
                                        </div>
                                        <input type="checkbox" name="settings[{{ $setting->key }}][value]" value="1" {{ $setting->value == '1' ? 'checked' : '' }}>
                                    </div>
                                    <input type="hidden" name="settings[{{ $setting->key }}][key]" value="{{ $setting->key }}">
                                @else
                                    <label class="block text-sm font-medium mb-2">{{ $setting->label ?? $setting->key }}</label>
                                    <input type="text" name="settings[{{ $setting->key }}][value]" value="{{ $setting->value }}" class="w-full px-3 py-2 border rounded">
                                    <input type="hidden" name="settings[{{ $setting->key }}][key]" value="{{ $setting->key }}">
                                    @if($setting->description)
                                        <p class="text-xs text-gray-500 mt-1">{{ $setting->description }}</p>
                                    @endif
...
function checkAllImageSync() {
    const imageKeys = @json(\App\Models\GeneralSetting::where('type', 'image')->pluck('key')->toArray());
...
function checkImageSync(key) {
    ...
    const optimizationBaseUrl = '{{ route('admin.settings.image.optimization') }}';
...
function loadOptimizationSuggestions(key = null) {
    const imageKeys = key ? [key] : @json(\App\Models\GeneralSetting::where('type', 'image')->pluck('key')->toArray());
    const optBaseUrl = '{{ route('admin.settings.image.optimization') }}';

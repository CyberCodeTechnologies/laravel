...
@section('title', 'Customer Analytics Reports - Admin - Panchi Gallery')
@section('header', 'Customer Analytics')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex justify-between items-center">
            ...
            <div class="flex gap-4 items-center">
                ...
                <a href="{{ route('admin.reports.export', 'customers') }}?period={{ $period }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
            ...
            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $customerSegments['new_customers'] }}</p>
            ...
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $customerSegments['returning_customers'] }}</p>
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $customerSegments['vip_customers'] }}</p>
            ...
            <div class="bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">${{ number_format($customerLifetimeValue, 2) }}</p>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="text-center p-6 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <p class="text-3xl font-bold text-green-900 mb-2">{{ $customerSegments['new_customers'] }}</p>
                ...
                <div class="text-center p-6 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <p class="text-3xl font-bold text-blue-900 mb-2">{{ $customerSegments['returning_customers'] }}</p>
                ...
                <div class="text-center p-6 bg-purple-50 rounded-lg border border-purple-200">
                    ...
                    <p class="text-3xl font-bold text-purple-900 mb-2">{{ $customerSegments['vip_customers'] }}</p>
                    ...
                    <tbody>
                        @foreach($topCustomersBySpending as $customer)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="py-3 text-sm font-medium">${{ number_format($customer->total_spent, 2) }}</td>
                                <td class="py-3 text-sm">{{ $customer->purchases }}</td>
                                <td class="py-3 text-sm">${{ number_format($customer->total_spent / $customer->purchases, 2) }}</td>
                                <td class="py-3">
                                    <span class="px-2 py-1 text-xs rounded-full 
                                        {{ $customer->purchases >= 5 ? 'bg-purple-100 text-purple-800' : 
                                           ($customer->purchases >= 2 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800') }}">
                                        {{ $customer->purchases >= 5 ? 'VIP' : 
                ...
                <div class="space-y-4">
                    <div>
                        <div class="flex justify-between mb-1">
                            ...
                            <span class="text-sm font-medium">{{ number_format($avgPurchaseFrequency, 1) }} purchases</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-600 h-2 rounded-full" style="width: {{ min(($avgPurchaseFrequency / 10) * 100, 100) }}%"></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex justify-between mb-1">
                            <span class="text-sm text-gray-600">Cart Abandonment Rate</span>
                            <span class="text-sm font-medium">{{ number_format($cartAbandonmentRate, 1) }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-red-600 h-2 rounded-full" style="width: {{ $cartAbandonmentRate }}%"></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex justify-between mb-1">
                            <span class="text-sm text-gray-600">Wishlist Activity</span>
                            <span class="text-sm font-medium">{{ $wishlistToPurchaseRate }} items</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-purple-600 h-2 rounded-full" style="width: {{ min(($wishlistToPurchaseRate / 100) * 100, 100) }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const acquisitionData = @json($customerAcquisition->map(function($item) {
        return [
            $item->date,
            $item->customers
        ];
    ...
    document.getElementById('periodFilter').addEventListener('change', function() {
        ...
        window.location.href = `{{ route('admin.reports.customers') }}?period=` + period;
    });
});
</script>
@endpush

...
@section('title', 'Marketplace Activity Reports - Admin - Panchi Gallery')
@section('header', 'Marketplace Activity Analytics')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex justify-between items-center">
            ...
            <div class="flex gap-4 items-center">
                ...
                <a href="{{ route('admin.reports.export', 'marketplace') }}?period={{ $period }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $pendingListings }}</p>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6 mb-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @foreach($resalesByStatus as $status)
                    <div class="text-center p-4 bg-{{ 
                        $status->status === 'listed' ? 'green' : 
                        ($status->status === 'sold' ? 'blue' : 
                        ($status->status === 'pending' ? 'yellow' : 'gray')) 
                        ...
                        <p class="text-sm text-gray-600 mt-1">{{ ucfirst($status->status) }}</p>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-lg font-bold text-gray-900">${{ number_format($avgListingPrice, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">Average Sold Price</span>
                        <span class="text-lg font-bold text-green-600">${{ number_format($avgSoldPrice, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">Price Difference</span>
                        <span class="text-lg font-bold {{ $avgSoldPrice > $avgListingPrice ? 'text-green-600' : 'text-red-600' }}">
                            {{ $avgSoldPrice > $avgListingPrice ? '+' : '-' }}${{ number_format(abs($avgSoldPrice - $avgListingPrice), 2) }}
                        </span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">Average Time to Sell</span>
                        <span class="text-lg font-bold text-blue-600">{{ $timeToSell ? round($timeToSell) : 'N/A' }} days</span>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    <div>
                        <div class="flex justify-between mb-1">
                            ...
                            <span class="text-sm font-medium">{{ $totalListings > 0 ? number_format(($soldListings / $totalListings) * 100, 1) : 0 }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-green-600 h-2 rounded-full" style="width: {{ $totalListings > 0 ? ($soldListings / $totalListings) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex justify-between mb-1">
                            <span class="text-sm text-gray-600">Active Rate</span>
                            <span class="text-sm font-medium">{{ $totalListings > 0 ? number_format(($activeListings / $totalListings) * 100, 1) : 0 }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-600 h-2 rounded-full" style="width: {{ $totalListings > 0 ? ($activeListings / $totalListings) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex justify-between mb-1">
                            <span class="text-sm text-gray-600">Pending Rate</span>
                            <span class="text-sm font-medium">{{ $totalListings > 0 ? number_format(($pendingListings / $totalListings) * 100, 1) : 0 }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-yellow-600 h-2 rounded-full" style="width: {{ $totalListings > 0 ? ($pendingListings / $totalListings) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6 mb-8">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full">
                    ...
                    <tbody>
                        @foreach($topResellingArtists as $artist)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="py-3 text-sm">{{ $artist->listings }}</td>
                                <td class="py-3 text-sm">{{ $artist->sold }}</td>
                                <td class="py-3 text-sm">
                                    <span class="px-2 py-1 text-xs rounded-full 
                                        {{ ($artist->sold / $artist->listings) * 100 >= 50 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                        {{ number_format(($artist->sold / $artist->listings) * 100, 1) }}%
                                    </span>
                                </td>
                                <td class="py-3">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($mostResoldArtworks as $artwork)
                    <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                        <div class="flex items-center justify-between mb-2">
                            <h4 class="font-medium text-gray-900 text-sm">{{ $artwork->title }}</h4>
                            <span class="px-2 py-1 text-xs bg-purple-100 text-purple-800 rounded-full">
                                {{ $artwork->resale_count }}x
                            </span>
                        </div>
                        <p class="text-xs text-gray-500">Resold {{ $artwork->resale_count }} times</p>
                        <div class="mt-2">
                            <div class="w-full bg-gray-200 rounded-full h-1">
                                <div class="bg-purple-600 h-1 rounded-full" style="width: {{ ($artwork->resale_count / $mostResoldArtworks->first()->resale_count) * 100 }}%"></div>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-lg p-6">
                ...
                <p class="text-3xl font-bold text-blue-900">{{ $activeListings }}</p>
            ...
            <div class="bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-lg p-6">
                ...
                <p class="text-3xl font-bold text-green-900">{{ $soldListings }}</p>
            ...
            <div class="bg-gradient-to-r from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-6">
                ...
                <p class="text-3xl font-bold text-purple-900">{{ $timeToSell ? round($timeToSell) : 'N/A' }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const listingData = @json($resaleListings->map(function($item) {
        return [
            $item->date,
            $item->listings
        ];
    ...
    document.getElementById('periodFilter').addEventListener('change', function() {
        ...
        window.location.href = `{{ route('admin.reports.marketplace') }}?period=` + period;
    });
});
</script>
@endpush

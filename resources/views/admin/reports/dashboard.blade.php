...
@section('title', 'Reports Dashboard - Admin - Panchi Gallery')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">${{ number_format($revenueThisMonth, 2) }}</p>
                        <p class="text-sm mt-2">
                            @if($revenueLastMonth > 0)
                                {{ (($revenueThisMonth - $revenueLastMonth) / $revenueLastMonth * 100) > 0 ? '+' : '' }}
                                {{ number_format(($revenueThisMonth - $revenueLastMonth) / $revenueLastMonth * 100, 1) }}% vs last month
                            @else
                                No previous data
                            @endif
                        </p>
            ...
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm mt-2">+{{ $newUsersThisMonth }} this month</p>
                    </div>
                    <div class="text-4xl opacity-80">👥</div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm mt-2">{{ $approvedArtworks }} approved</p>
                    </div>
                    <div class="text-4xl opacity-80">🎨</div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm mt-2">{{ $pendingTransactions }} pending</p>
                    </div>
                    <div class="text-4xl opacity-80">💳</div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div class="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <p class="text-2xl font-bold text-gray-900">{{ $totalArtists }}</p>
                <p class="text-sm text-gray-600">{{ $approvedArtists }} approved</p>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <p class="text-2xl font-bold text-gray-900">{{ $activeResales }}</p>
                <p class="text-sm text-gray-600">{{ $pendingResales }} pending listings</p>
            </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center">
                                <div class="w-32 bg-gray-200 rounded-full h-2 mr-3">
                                    <div class="bg-blue-600 h-2 rounded-full" style="width: {{ $topCategories->first()->artworks_count > 0 ? ($category->artworks_count / $topCategories->first()->artworks_count) * 100 : 0 }}%"></div>
                                </div>
                                <span class="text-sm text-gray-600">{{ $category->artworks_count }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-4 bg-gray-50 rounded-lg">
                        <p class="text-2xl font-bold text-blue-600">{{ $totalLikes }}</p>
                        <p class="text-sm text-gray-600">Total Likes</p>
                    </div>
                    <div class="text-center p-4 bg-gray-50 rounded-lg">
                        <p class="text-2xl font-bold text-green-600">{{ $totalWishlists }}</p>
                        <p class="text-sm text-gray-600">Wishlists</p>
                    </div>
                    <div class="text-center p-4 bg-gray-50 rounded-lg">
                        <p class="text-2xl font-bold text-purple-600">{{ $activeCarts }}</p>
                        <p class="text-sm text-gray-600">Active Carts</p>
                    </div>
                    <div class="text-center p-4 bg-gray-50 rounded-lg">
                        <p class="text-2xl font-bold text-orange-600">{{ $soldArtworks }}</p>
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody>
                            @foreach($recentTransactions as $transaction)
                                <tr class="border-b">
                                    <td class="py-2 text-sm">{{ $transaction->transaction_id }}</td>
                                    <td class="py-2 text-sm">{{ $transaction->buyer->name }}</td>
                                    <td class="py-2 text-sm">${{ number_format($transaction->amount, 2) }}</td>
                                    <td class="py-2">
                    ...
                    @foreach($recentUsers as $user)
                        <div class="flex items-center justify-between py-2 border-b">
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-xs text-gray-500 mt-1">{{ $user->created_at->format('M d') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>

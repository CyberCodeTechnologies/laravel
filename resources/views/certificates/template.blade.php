...
<body>
    <div class="certificate">
        ...
        <div class="content">
            ...
            <div class="artwork-title">{{ $artwork->title ?? 'Artwork Title' }}</div>
            <p>is an authentic original artwork created by</p>
            <p style="font-size: 20px; font-weight: bold; margin: 20px 0;">{{ $artwork->artist->name ?? 'Artist Name' }}</p>
        </div>
        
        <div class="details">
            <p><strong>Certificate ID:</strong> {{ $certificate->certificate_code ?? 'CERT-' . strtoupper(Str::random(8)) }}</p>
            <p><strong>Medium:</strong> {{ $artwork->medium ?? 'Not specified' }}</p>
            <p><strong>Dimensions:</strong> {{ $artwork->dimensions ?? 'Not specified' }}</p>
            <p><strong>Year:</strong> {{ $artwork->year ?? date('Y') }}</p>
        ...
        <div class="qr-code">
            ...
            <p style="margin-top: 10px;">Certificate Code: {{ $certificate->certificate_code ?? 'CERT-' . strtoupper(Str::random(8)) }}</p>
        </div>

...
@section('title', 'Certificate Verification - Panchi Gallery')
            ...
            <div class="bg-white border border-black rounded-lg shadow-lg p-8 mb-8">
                <div class="grid md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        @if($certificate->artwork->images)
                            <div class="mb-6">
                                <img src="{{ $certificate->artwork->primary_image }}" 
                                     alt="{{ $certificate->artwork->title }}" 
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                ...
                                <span>{{ $certificate->artwork->title }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Artist:</span>
                                <span>{{ $certificate->artwork->artist->name }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Medium:</span>
                                <span class="capitalize">{{ $certificate->artwork->medium }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Dimensions:</span>
                                <span>{{ $certificate->artwork->dimensions }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Year:</span>
                                <span>{{ $certificate->artwork->year ?? 'N/A' }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Category:</span>
                                <span>{{ $certificate->artwork->category->name }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                ...
                                <span class="font-mono">{{ $certificate->certificate_code }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Issue Date:</span>
                                <span>{{ $certificate->issue_date->format('F j, Y') }}</span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Status:</span>
                                @php $isVerified = $certificate->is_verified; @endphp
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $isVerified ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $isVerified ? 'Verified' : 'Not Verified' }}
                                </span>
                            </div>
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="font-medium">Current Owner:</span>
                                <span>{{ $certificate->current_owner?->name ?? 'N/A' }}</span>
                            </div>
                        </div>
                        ...
                        @if($certificate->qr_code)
                            <div class="mt-6 text-center">
                                ...
                                <img src="{{ $certificate->qr_code_url }}" 
                ...
                <div class="mt-8 p-6 bg-gray-50 rounded-lg">
                    ...
                    <p class="whitespace-pre-line text-gray-700">{{ $certificate->certificate_text }}</p>
                </div>

                <!-- Ownership History -->
                @if($certificate->ownership_history)
                    <div class="mt-8">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full border-collapse">
                                ...
                                <tbody>
                                    @foreach($certificate->ownership_history as $ownership)
                                        <tr class="border-b border-gray-200">
                                            <td class="py-3 px-4">{{ $ownership['owner']['name'] }}</td>
                                            <td class="py-3 px-4">{{ \Carbon\Carbon::parse($ownership['acquired_at'])->format('M j, Y') }}</td>
                                            <td class="py-3 px-4">{{ $ownership['purchase_price'] ? '$' . number_format($ownership['purchase_price'], 2) : 'N/A' }}</td>
                                            <td class="py-3 px-4">{{ ucfirst($ownership['transaction_type']) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div class="mt-8 flex flex-wrap gap-4">
                    @if($certificate->certificate_pdf)
                        <a href="{{ $certificate->certificate_pdf_url }}" 
                           download="certificate-{{ $certificate->certificate_code }}.pdf"
        ...
        <div class="max-w-2xl mx-auto px-4 py-12 text-center">
            <div class="bg-white border border-black rounded-lg shadow-lg p-8">
                ...
                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
                    <p class="font-mono text-lg">{{ request('certificate_code') }}</p>
                </div>

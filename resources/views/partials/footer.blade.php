...
@endphp
                ...
                <div class="flex items-center gap-3">
                    <span class="text-gray-400 text-sm">{{ __('messages.follow_us') }}</span>
            ...
            <!-- Quick Links -->
            <div>
                <h3 class="font-serif text-lg font-semibold mb-6 text-white">{{ __('messages.explore') }}</h3>
                <ul class="space-y-3">
                    <li>
                        <a href="{{ route('public.artworks.index') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.all_artworks') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('public.artists.index') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.artists') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('public.exhibitions.index') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.exhibitions') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('blog.index') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.blog') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('marketplace.index') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.marketplace') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('custom-orders.create') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.custom_orders') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('collections') }}" class="text-white hover:text-gray-300 transition-colors">
            ...
            <!-- Support -->
            <div>
                <h3 class="font-serif text-lg font-semibold mb-6 text-white">{{ __('messages.support') }}</h3>
                <ul class="space-y-3">
                    <li>
                        <a href="{{ route('about') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.about_us') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('contact') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.contact') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('faq') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.faq') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('privacy') }}" class="text-white hover:text-gray-300 transition-colors">
                            {{ __('messages.privacy_policy') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('terms') }}" class="text-white hover:text-gray-300 transition-colors">
        ...
        <div class="mt-16 pt-8 border-t border-gray-800">
            <div class="max-w-2xl mx-auto text-center">
                <h3 class="font-serif text-xl font-semibold mb-4 text-white">{{ __('messages.stay_updated') }}</h3>
                <p class="text-gray-300 mb-6 leading-relaxed">
                    {{ __('messages.get_exclusive_access') }}
                </p>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                <div class="flex flex-col items-center md:items-start gap-3">
                    ...
                    <div class="flex items-center gap-4">
                        <span class="text-gray-500 text-xs">{{ __('messages.secure_payment_label') }}</span>
                        <!-- Visa -->
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div class="relative group">
                        <button class="flex items-center space-x-2 text-gray-400 hover:text-white transition-colors text-sm">
                            ...
                            <span class="text-xs font-medium">{{ app()->getLocale() === 'en' ? __('messages.english') : __('messages.myanmar') }}</span>
                    ...
                    <div class="relative group">
                        <button class="flex items-center space-x-2 text-gray-400 hover:text-white transition-colors text-sm">
                            ...
                            <span class="text-xs font-medium currency-display">{{ $currentCurrency ?? 'USD' }}</span>
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div class="absolute bottom-full mb-2 right-0 w-28 bg-white border border-gray-200 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                            @foreach($supportedCurrencies as $code => $config)
                                @if($code !== ($currentCurrency ?? 'USD'))
                                    <button onclick="switchCurrency('{{ $code }}')" class="block w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 rounded-t-xl transition-colors">
                                        {{ $config['symbol'] }} {{ $code }}
                                    </button>
                                @endif
                            @endforeach

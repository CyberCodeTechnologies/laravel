...
@section('title')
    {{ __('messages.our_artists') . ' - Panchi Gallery' }}
@endsection

@section('meta-description')
    {{ __('messages.artists_meta_description') }}
@endsection
...
<script type="application/ld+json">
{
    ...
    "description": "{{ __('messages.artists_meta_description') }}"
}
</script>
@endsection
                ...
                <div class="relative group">
                    ...
                    <div class="relative">
                        <input 
                            type="text" 
                            id="artistSearch" 
                            placeholder="{{ __('messages.search_artists_placeholder') }}"
        ...
        <div class="flex flex-col md:flex-row items-center justify-between mb-16 gap-6">
            <div>
                <h2 class="font-serif text-4xl md:text-5xl font-bold text-gray-900 mb-4">
                    {{ __('messages.featured_artists') }}
                </h2>
                <p class="text-gray-600 text-lg">{{ __('messages.featured_artists_subtitle') }}</p>
            </div>
            <div class="flex items-center space-x-4">
                <div class="relative">
                    <select class="appearance-none px-6 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:border-black focus:ring-2 focus:ring-black/10 transition-all duration-300 cursor-pointer pr-10 shadow-sm">
                        <option>{{ __('messages.all_specializations') }}</option>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
            <div class="text-center group">
                ...
                <div class="text-gray-600 text-lg font-medium">{{ __('messages.artists') }}</div>
            </div>
            <div class="text-center group">
                <div class="text-5xl md:text-6xl font-bold text-gray-900 mb-2 group-hover:scale-110 transition-transform duration-300">
                    {{ $artists->sum('artworks_count') ?? 0 }}+
                </div>
                <div class="text-gray-600 text-lg font-medium">{{ __('messages.artworks') }}</div>
            </div>
            <div class="text-center group">
                <div class="text-5xl md:text-6xl font-bold text-gray-900 mb-2 group-hover:scale-110 transition-transform duration-300">
                    {{ $artists->sum('followers_count') ?? 0 }}+
                </div>
                <div class="text-gray-600 text-lg font-medium">{{ __('messages.followers_count') }}</div>
            </div>
            <div class="text-center group">
                <div class="text-5xl md:text-6xl font-bold text-gray-900 mb-2 group-hover:scale-110 transition-transform duration-300">
                    10+
                </div>
                <div class="text-gray-600 text-lg font-medium">{{ __('messages.years') }}</div>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="text-center mb-16">
            <div class="inline-flex items-center space-x-2 bg-black/5 backdrop-blur-md px-6 py-2 rounded-full mb-6 border border-black/10">
                <span class="w-2 h-2 bg-black rounded-full animate-pulse"></span>
                <span class="text-gray-800 text-sm font-medium">{{ __('messages.artist_spotlight') }}</span>
            </div>
            <h2 class="font-serif text-5xl md:text-6xl font-bold text-gray-900 mb-4">
                {{ __('messages.featured_artist_description') }}
            </h2>
            <p class="text-xl text-gray-600 max-w-2xl mx-auto">
                {{ __('messages.discover_featured_talent') }}
            </p>
        </div>
        ...
        <div class="bg-white rounded-3xl overflow-hidden border border-gray-200 shadow-2xl transform hover:scale-[1.02] transition-transform duration-500">
            <div class="grid grid-cols-1 lg:grid-cols-2">
                <div class="relative h-[500px] lg:h-[600px] overflow-hidden">
                    ...
                    <img src="{{ $featuredArtist->avatar_url }}" 
                         alt="{{ $featuredArtist->name }}" 
                         class="w-full h-full object-cover transition-transform duration-700 hover:scale-105">
                    <div class="absolute bottom-0 left-0 right-0 p-8 lg:p-12 z-20">
                        <div class="flex items-center space-x-3 mb-4">
                            <div class="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
                            <span class="text-white/90 text-sm font-medium">{{ __('messages.available_for_commissions') }}</span>
                        </div>
                        <h3 class="font-serif text-4xl md:text-5xl font-bold text-white mb-3">{{ $featuredArtist->name }}</h3>
                        <p class="text-xl text-white/90">{{ $featuredArtist->specialization ?? __('messages.contemporary_artist') }}</p>
                    </div>
                </div>
                ...
                <div class="p-8 lg:p-16 flex flex-col justify-center bg-gradient-to-b from-gray-50 to-white">
                    <h3 class="font-serif text-2xl md:text-3xl font-semibold text-gray-900 mb-8 italic leading-relaxed">
                        "{{ $featuredArtist->bio ? Str::limit($featuredArtist->bio, 150) : __('messages.artist_quote_default') }}"
                    </h3>
                    <p class="text-gray-600 mb-10 leading-relaxed text-lg">
                        {{ $featuredArtist->bio ?? __('messages.artist_bio_default') }}
                    </p>
                    ...
                    <div class="grid grid-cols-3 gap-6 mb-10">
                        <div class="text-center p-6 bg-gray-50 rounded-2xl border border-gray-200 group hover:bg-gray-100 transition-all duration-300">
                            <div class="text-4xl font-bold text-gray-900 mb-2 group-hover:scale-110 transition-transform">{{ $featuredArtist->artworks_count ?? 0 }}</div>
                            <div class="text-gray-600 text-sm font-medium">{{ __('messages.artworks_count') }}</div>
                        </div>
                        <div class="text-center p-6 bg-gray-50 rounded-2xl border border-gray-200 group hover:bg-gray-100 transition-all duration-300">
                            <div class="text-4xl font-bold text-gray-900 mb-2 group-hover:scale-110 transition-transform">{{ $featuredArtist->followers_count ?? 0 }}</div>
                            <div class="text-gray-600 text-sm font-medium">{{ __('messages.followers_count') }}</div>
                        </div>
                        <div class="text-center p-6 bg-gray-50 rounded-2xl border border-gray-200 group hover:bg-gray-100 transition-all duration-300">
                            <div class="text-4xl font-bold text-gray-900 mb-2 group-hover:scale-110 transition-transform">{{ $featuredArtist->years_active ?? 5 }}+</div>
                            <div class="text-gray-600 text-sm font-medium">{{ __('messages.years_active') }}</div>
                        </div>
                    </div>
                    
                    <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                        <a href="{{ route('public.artists.show', $featuredArtist->slug ?? $featuredArtist->id) }}" class="group relative px-10 py-4 rounded-xl font-semibold inline-flex items-center justify-center space-x-2 bg-black text-white shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                            <span class="relative z-10">{{ __('messages.view_portfolio') }}</span>
                        ...
                        @auth
                            @if(auth()->id() !== $featuredArtist->id)
                            <button onclick="toggleFollow({{ $featuredArtist->id }})" class="px-10 py-4 rounded-xl font-semibold border-2 border-black text-black hover:bg-black hover:text-white transition-all duration-300">
                                {{ auth()->user()->isFollowing($featuredArtist) ? __('messages.following') : __('messages.follow_artist') }}
                            </button>
                            @endif
                        @endauth
                    </div>
...
function toggleFollow(artistId) {
    ...
    .then(data => {
        if (data.success) {
            // Update button state
            if (data.following) {
                button.textContent = '{{ __("messages.following") }}';
                ...
                button.textContent = '{{ __("messages.follow") }}';

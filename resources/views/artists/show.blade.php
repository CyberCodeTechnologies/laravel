...
@section('title')
    {{ $artist->name . ' - Panchi Gallery' }}
@endsection

@section('meta-description')
    Discover artworks by {{ $artist->name }}, {{ $artist->specialization ?? 'Myanmar Artist' }}. Browse their collection and learn about their artistic journey.
@endsection

@section('meta-keywords')
    {{ $artist->name }}, Myanmar artist, {{ $artist->specialization ?? 'contemporary art' }}, {{ $artist->location ?? 'Myanmar' }}, original artwork, art gallery, fine art
@endsection

@section('meta-image')
    {{ $artist->avatar_url ?? asset('images/og-default.jpg') }}
@endsection
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ol class="flex items-center space-x-2 text-sm" itemscope itemtype="https://schema.org/BreadcrumbList">
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <a href="{{ route('home') }}" class="text-gray-500 hover:text-black transition-colors" itemprop="item">
            ...
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <a href="{{ route('public.artists.index') }}" class="text-gray-500 hover:text-black transition-colors" itemprop="item">
                    <span itemprop="name">{{ __('messages.artists') }}</span>
                </a>
                <meta itemprop="position" content="2">
            </li>
            <li class="text-gray-300">/</li>
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <span class="text-gray-900 font-medium" itemprop="name">{{ $artist->name }}</span>
...
<section class="relative h-80 md:h-[32rem] overflow-hidden">
    <div class="absolute inset-0">
        <img src="{{ $artist->cover_image_url ?? asset('images/placeholder-artist-cover.jpg') }}" 
             alt="{{ $artist->name }}" 
            ...
            <div class="flex flex-col md:flex-row items-end md:items-center space-y-4 md:space-y-0 md:space-x-6">
                ...
                <div class="flex-1 text-center md:text-left">
                    <div class="flex flex-col md:flex-row md:items-center md:space-x-4">
                        <h1 class="font-serif text-3xl md:text-5xl font-bold mb-2 md:mb-0">{{ $artist->name }}</h1>
                        <div class="flex items-center space-x-2">
                            ...
                            @endif
                            @if($artist->location)
                                <span class="px-3 py-1 bg-white/20 backdrop-blur-sm text-white text-xs font-medium rounded-full">{{ $artist->location }}</span>
                            @endif
                        </div>
                    </div>
                    <p class="text-lg md:text-xl opacity-90 font-light mt-2">{{ $artist->specialization ?? __('messages.visual_artist') }}</p>
                    @if($artist->bio)
                        <p class="text-sm md:text-base opacity-80 mt-2 line-clamp-2">{{ Str::limit($artist->bio, 150) }}</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="absolute top-4 right-4 flex flex-col space-y-2">
        ...
        @if(auth()->check() && auth()->id() !== $artist->id)
            <button onclick="toggleFollow({{ $artist->id }})" class="p-3 bg-white/20 backdrop-blur-sm text-white rounded-full hover:bg-white/30 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            <div class="bg-white rounded-xl p-4 text-center shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                <div class="text-2xl md:text-3xl font-bold text-gray-900">{{ $artist->artworks_count ?? 0 }}</div>
                <div class="text-xs text-gray-500 uppercase tracking-wide mt-1">Artworks</div>
            </div>
            <div class="bg-white rounded-xl p-4 text-center shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                <div class="text-2xl md:text-3xl font-bold text-gray-900">{{ $artist->followers_count ?? 0 }}</div>
                <div class="text-xs text-gray-500 uppercase tracking-wide mt-1">Followers</div>
            </div>
            <div class="bg-white rounded-xl p-4 text-center shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                <div class="text-2xl md:text-3xl font-bold text-gray-900">{{ $artist->years_active ?? 0 }}</div>
                <div class="text-xs text-gray-500 uppercase tracking-wide mt-1">Years Active</div>
            </div>
            <div class="bg-white rounded-xl p-4 text-center shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                <div class="text-2xl md:text-3xl font-bold text-gray-900">{{ $artist->artworks->sum('views_count') ?? 0 }}</div>
            ...
            <div class="lg:col-span-1">
                ...
                <div class="mb-8">
                    ...
                    <div class="prose prose-sm max-w-none">
                        ...
                        @else
                            <p class="text-gray-500 italic">{{ __('messages.artist_bio_default') }}</p>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                            <div class="flex items-center text-gray-700">
                                ...
                                <span class="font-medium">{{ $artist->location }}</span>
                            </div>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            @if(($artist->social_links['instagram'] ?? null) && is_string($artist->social_links['instagram']))
                                <a href="{{ $artist->social_links['instagram'] }}" target="_blank" class="flex items-center justify-center p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg hover:from-purple-100 hover:to-pink-100 transition-all group">
                            ...
                            @if(($artist->social_links['facebook'] ?? null) && is_string($artist->social_links['facebook']))
                                <a href="{{ $artist->social_links['facebook'] }}" target="_blank" class="flex items-center justify-center p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg hover:from-blue-100 hover:to-indigo-100 transition-all group">
                            ...
                            @endif
                            @if(($artist->social_links['website'] ?? null) && is_string($artist->social_links['website']))
                                <a href="{{ $artist->social_links['website'] }}" target="_blank" class="flex items-center justify-center p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg hover:from-green-100 hover:to-emerald-100 transition-all group">
                ...
                <div class="mb-8 p-6 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl text-center shadow-xl border border-gray-700">
                    ...
                    <div class="flex justify-center mb-4">
                        <div class="bg-white p-4 rounded-xl shadow-inner hover:shadow-lg transition-shadow">
                            <img src="{{ $qrCodeUrl }}" 
                                 alt="{{ __('messages.artist_profile_qr_code') }}" 
                                 class="rounded-lg w-44 h-44">
                        </div>
                    </div>
                    <p class="text-xs text-gray-400 mb-3">{{ __('messages.scan_to_view_profile', ['name' => $artist->name]) }}</p>
                ...
                <div class="flex flex-col space-y-3">
                    @if(auth()->check() && auth()->id() !== $artist->id)
                        <button onclick="toggleFollow({{ $artist->id }})" 
                            ...
                            {{ auth()->user()->isFollowing($artist) ? __('messages.following') : __('messages.follow_artist') }}
                        </button>
                    @endif
                    
                    <a href="{{ route('contact', ['artist' => $artist->slug]) }}" 
                ...
                <div class="border-t border-gray-200 pt-6 mt-6">
                    <p class="text-sm font-medium mb-3 text-gray-700">Share Artist Profile</p>
                    <x-social-share 
                        :url="route('artists.show', $artist->slug)"
                        :title="$artist->name . ' - Myanmar Artist'"
                        :description="$artist->bio ?? 'Discover artworks by ' . $artist->name . ' at Panchi Gallery'"
                        :image="$artist->avatar"
                    />
                </div>
            </div>
            ...
            <div class="lg:col-span-2">
                ...
                <div class="mb-8">
                    <nav class="flex space-x-1 bg-gradient-to-r from-gray-50 to-gray-100 p-1.5 rounded-xl border border-gray-200">
                        <button onclick="switchTab('artworks')" id="artworks-tab" class="flex-1 py-3 px-4 rounded-lg font-medium text-sm transition-all bg-white text-gray-900 shadow-sm">
                            {{ __('messages.artworks') }}
                            <span class="ml-1.5 text-xs bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-2 py-0.5 rounded-full" id="artworks-count">{{ $artist->artworks_count ?? 0 }}</span>
                        </button>
                ...
                <div id="artworks-content" class="tab-content">
                    ...
                    <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                        <div class="flex flex-wrap gap-2">
                            <button onclick="filterArtworks('all')" id="filter-all" class="filter-btn px-4 py-2 bg-purple-600 border border-purple-600 rounded-lg text-sm font-medium text-white hover:bg-purple-700 transition-colors">
                                All
                                <span class="ml-1.5 text-xs bg-white text-purple-600 px-1.5 py-0.5 rounded-full">{{ $artist->artworks_count ?? 0 }}</span>
                            </button>
                            <button onclick="filterArtworks('available')" id="filter-available" class="filter-btn px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                                Available
                                <span class="ml-1.5 text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full">{{ $artist->available_count ?? 0 }}</span>
                            </button>
                            <button onclick="filterArtworks('sold')" id="filter-sold" class="filter-btn px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                                Sold
                                <span class="ml-1.5 text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full">{{ $artist->sold_count ?? 0 }}</span>
                            </button>
                        </div>
                        <div class="flex items-center space-x-2">
                    ...
                    @if($artist->artworks && $artist->artworks->count() > 0)
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6" id="artworks-grid"
                             ...
                             data-current-page="1"
                             data-has-more="{{ $artist->artworks_count > $artist->artworks->count() ? 'true' : 'false' }}">
                            @foreach($artist->artworks as $artwork)
                                <x-artwork-card :artwork="$artwork" class="hover:shadow-2xl transition-all duration-300 transform hover:scale-105" />
                            @endforeach
                        </div>
                        ...
                        <div id="no-artworks-message" class="hidden text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl border border-gray-200">
                            ...
                            <p class="text-gray-600 text-lg mb-2" id="no-artworks-text">{{ __('messages.no_artworks_yet') }}</p>
                        </div>

                        @if($artist->artworks_count > $artist->artworks->count())
                            <div class="text-center mt-12" id="load-more-container">
                                <button id="load-more-btn" onclick="loadMoreArtworks()" class="px-8 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl font-medium hover:from-purple-700 hover:to-indigo-700 transition-all transform hover:scale-[1.02] shadow-lg hover:shadow-xl">
                                    {{ __('messages.load_more_artworks') }}
                                </button>
                            </div>
                        @endif
                    @else
                        <div class="text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl border border-gray-200">
                            ...
                            <p class="text-gray-600 text-lg mb-2">{{ __('messages.no_artworks_yet') }}</p>
                            <p class="text-gray-500 text-sm">Check back soon for new artworks from {{ $artist->name }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div id="about-content" class="tab-content hidden">
                    <div class="prose max-w-none">
                        ...
                        @if($artist->education && is_array($artist->education) && count($artist->education) > 0)
                            <div class="mb-8">
                                <h3 class="font-serif text-2xl font-semibold mb-4 flex items-center">
                                    ...
                                    {{ __('messages.education_training') }}
                                </h3>
                                <div class="space-y-3">
                        ...
                        @if(!$artist->artist_statement && (!$artist->education || count($artist->education) == 0) && (!$artist->awards || count($artist->awards) == 0))
                            <div class="text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl border border-gray-200">
                                <svg class="w-20 h-20 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                </svg>
                                <p class="text-gray-600 text-lg mb-2">{{ __('messages.detailed_info_coming_soon') }}</p>
                                <p class="text-gray-500 text-sm">We're working on bringing you more detailed information about {{ $artist->name }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div id="exhibitions-content" class="tab-content hidden">
                    @if($artist->exhibitions && is_array($artist->exhibitions) && count($artist->exhibitions) > 0)
                        <div class="space-y-6">
                            @foreach($artist->exhibitions as $exhibition)
                                <div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6 border border-indigo-200 hover:border-indigo-300 transition-all hover:shadow-lg">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <h4 class="font-semibold text-gray-900 mb-2 text-lg">{{ $exhibition['title'] ?? 'Exhibition' }}</h4>
                                            <div class="flex items-center text-gray-600 mb-2">
                                                <svg class="w-4 h-4 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                                </svg>
                                                {{ $exhibition['venue'] ?? 'Venue TBD' }}
                                            </div>
                                            <div class="flex items-center text-sm text-gray-500">
                                                <svg class="w-4 h-4 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                </svg>
                                                {{ $exhibition['year'] ?? date('Y') }}
                                            </div>
                                        </div>
                                        <div class="ml-4">
                    ...
                    @else
                        <div class="text-center py-16 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-200">
                            ...
                            <p class="text-gray-600 text-lg mb-2">{{ __('messages.exhibition_history_coming_soon') }}</p>
                            <p class="text-gray-500 text-sm">{{ $artist->name }}'s exhibition history will be available soon</p>
                        </div>
                    @endif
                </div>
                
                <div id="press-content" class="tab-content hidden">
                    @if($artist->press && is_array($artist->press) && count($artist->press) > 0)
                        <div class="space-y-6">
                            @foreach($artist->press as $article)
                                <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200 hover:border-green-300 transition-all hover:shadow-lg">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <h4 class="font-semibold text-gray-900 mb-2 text-lg">{{ $article['title'] ?? 'Press Article' }}</h4>
                                            <div class="flex items-center text-gray-600 mb-2">
                                        ...
                                        <div class="ml-4">
                                            @if($article['url'] ?? null)
                                                <a href="{{ $article['url'] }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors">
                    ...
                    @else
                        <div class="text-center py-16 bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl border border-green-200">
                            ...
                            <p class="text-gray-600 text-lg mb-2">{{ __('messages.press_coverage_coming_soon') }}</p>
                            <p class="text-gray-500 text-sm">Media coverage and press releases for {{ $artist->name }} will be available soon</p>
                        </div>
                    @endif
                </div>
            </div>
...
function toggleFollow(artistId) {
    ...
    fetch(`{{ route('public.artists.follow', ':id') }}`.replace(':id', artistId), {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            button.innerHTML = data.following 
                ? '<svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>{{ __('messages.following') }}'
                : '<svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>{{ __('messages.follow_artist') }}';
...
function downloadQRCode() {
    ...
    link.href = qrUrl;
    link.download = '{{ $artist->slug }}-qrcode.png';
    link.target = '_blank';
...
function subscribeToArtist() {
    ...
    setTimeout(() => {
        showToast('Successfully subscribed to {{ $artist->name }}\'s updates!', 'success');
    }, 1000);
}
...
function filterArtworks(filter) {
    ...
    if (visibleCount === 0) {
        ...
        if (filter === 'available') {
            ...
            noArtworksText.textContent = '{{ __('messages.no_artworks_yet') }}';
        }
    } else {

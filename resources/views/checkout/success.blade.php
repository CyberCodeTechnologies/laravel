...
@section('title', __('messages.order_success_title') . ' - Panchi Gallery')
@section('meta-description', __('messages.order_success_meta'))
...
<div class="min-h-screen bg-gray-50 py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12">
            ...
            <h1 class="text-4xl font-bold text-gray-900 mb-4">{{ __('messages.order_placed_success') }}</h1>
        ...
        @if($order)
            <div class="bg-white rounded-lg shadow-sm p-8 mb-8">
                <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('messages.order_details') }}</h2>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <!-- Order Information -->
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('messages.order_information') }}</h3>
                        <div class="space-y-2 text-gray-600">
                            ...
                            <p><strong>{{ __('messages.total_amount') }}:</strong> {{ $order->formatted_total }}</p>
                            <p><strong>{{ __('messages.payment_method') }}:</strong> {{ __('messages.' . strtolower($order->payment_method)) }}</p>
                            <p><strong>{{ __('messages.shipping_method') }}:</strong> {{ __('messages.' . ($order->shipping_method ?? 'standard')) }}</p>
                        </div>
                    </div>

                    <!-- Shipping Information -->
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('messages.shipping_information') }}</h3>
                        <div class="space-y-2 text-gray-600">
                            <p><strong>{{ __('messages.name') }}:</strong> {{ $order->full_name }}</p>
                            ...
                            @foreach($order->items as $item)
                                <div class="flex items-center space-x-4 p-4 border border-gray-200 rounded-lg">
                                    <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                                        @if($item->artwork && $item->artwork->primary_image)
                                    ...
                                    <div class="flex-1">
                                        <h4 class="font-medium text-gray-900">{{ $item->artwork_title }}</h4>
                                        <p class="text-sm text-gray-600">{{ __('messages.quantity') }}: {{ $item->quantity }}</p>
                                        <p class="text-sm text-gray-600">{{ $item->formatted_price }} {{ __('messages.each') }}</p>
                                    </div>
        ...
        @else
            <div class="bg-white rounded-lg shadow-sm p-8 mb-8 text-center">
                <p class="text-gray-600">{{ __('messages.order_details_not_available') }}</p>
            </div>
        @endif

        <!-- Next Steps -->
        <div class="bg-white rounded-lg shadow-sm p-8 mb-8">
            <h2 class="text-xl font-bold text-gray-900 mb-6">{{ __('messages.whats_next') }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        <h4 class="font-medium text-gray-900">{{ __('messages.order_confirmation') }}</h4>
                        <p class="text-sm text-gray-600">{{ __('messages.email_confirmation_shortly') }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        <h4 class="font-medium text-gray-900">{{ __('messages.artwork_processing') }}</h4>
                        <p class="text-sm text-gray-600">{{ __('messages.team_prepare_shipping') }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        <h4 class="font-medium text-gray-900">{{ __('messages.certificate_of_authenticity') }}</h4>
                        <p class="text-sm text-gray-600">{{ __('messages.includes_verified_certificate') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="text-center space-y-4">
            ...
            @if(auth()->check())
                <a href="{{ route('collector.purchases') }}" 

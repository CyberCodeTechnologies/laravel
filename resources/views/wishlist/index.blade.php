...
@section('title', 'Wishlist - Panchi Gallery')
@section('meta-description', 'View and manage your wishlist of artworks from Panchi Gallery.')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if(auth()->check() && auth()->user()->wishlistItems->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach(auth()->user()->wishlistItems as $wishlistItem)
                    @if($wishlistItem->artwork)
                        <div class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                            <!-- Artwork Image -->
                            <div class="aspect-square bg-gray-100">
                                @if($wishlistItem->artwork->image)
                                    <img src="{{ asset('storage/' . $wishlistItem->artwork->image) }}" 
                                         alt="{{ $wishlistItem->artwork->title }}"
                            ...
                            <!-- Artwork Info -->
                            <div class="p-4">
                                <h3 class="font-semibold text-gray-900 mb-1">{{ $wishlistItem->artwork->title }}</h3>
                                <p class="text-sm text-gray-600 mb-2">{{ $wishlistItem->artwork->artist->name ?? 'Unknown Artist' }}</p>
                                <div class="flex items-center justify-between">
                                    <span class="text-lg font-bold text-gray-900">
                                        {{ $wishlistItem->artwork->formatted_price ?? '$0.00' }}
                                    </span>
                                    <div class="flex space-x-2">
                                        ...
                                        <!-- Add to Cart Button -->
                                        <button type="button"
                                                onclick="buyNow({{ $wishlistItem->artwork->id }})"

...
<?php $__env->startSection('title', 'Navigation Menu - Panchi Gallery'); ?>
<?php $__env->startSection('header', 'Navigation Menu'); ?>
...
<div class="space-y-6">
    ...
    <form action="<?php echo e(route('admin.frontend.navigation.update')); ?>" method="POST" class="card-luxury rounded-xl p-6 space-y-4">
        <?php echo csrf_field(); ?>
        <?php
            $defaults = [
        ...
        <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 bg-gray-50 rounded-lg items-end">
                <input type="hidden" name="menu_items[<?php echo e($index); ?>][key]" value="<?php echo e($item->key); ?>">
                <div class="md:col-span-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Key</label>
                    <input type="text" value="<?php echo e($item->key); ?>" disabled class="w-full border-gray-300 rounded-lg bg-white">
                </div>
                <div class="md:col-span-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Label (EN)</label>
                    <input type="text" name="menu_items[<?php echo e($index); ?>][content_en]" value="<?php echo e(old("menu_items.{$index}.content_en", $item->content_en)); ?>" required class="w-full border-gray-300 rounded-lg">
                </div>
                <div class="md:col-span-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Label (MY)</label>
                    <input type="text" name="menu_items[<?php echo e($index); ?>][content_my]" value="<?php echo e(old("menu_items.{$index}.content_my", $item->content_my)); ?>" class="w-full border-gray-300 rounded-lg">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Order</label>
                    <input type="number" name="menu_items[<?php echo e($index); ?>][sort_order]" value="<?php echo e(old("menu_items.{$index}.sort_order", $item->sort_order ?? $index)); ?>" class="w-full border-gray-300 rounded-lg">
                </div>
                <div class="md:col-span-1">
                    <label class="inline-flex items-center gap-2 text-sm">
                        <input type="hidden" name="menu_items[<?php echo e($index); ?>][is_active]" value="0">
                        <input type="checkbox" name="menu_items[<?php echo e($index); ?>][is_active]" value="1" <?php echo e(old("menu_items.{$index}.is_active", $item->is_active ?? true) ? 'checked' : ''); ?>>
                        Active
                    </label>
                </div>
            </div>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\panchigallery.com\resources\views\admin\frontend\navigation.blade.php ENDPATH**/ ?>
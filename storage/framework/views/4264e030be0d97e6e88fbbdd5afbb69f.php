...
<?php $__env->startSection('title', 'Manage Wishlists - Admin'); ?>
<?php $__env->startSection('meta-description', 'Manage user wishlists on Panchi Gallery platform'); ?>

<?php $__env->startSection('header', 'Manage Wishlists'); ?>
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-blue-900"><?php echo e(App\Models\Wishlist::count()); ?></p>
                        <p class="text-xs text-blue-700 mt-1">
                            <?php
                                $uniqueUsers = App\Models\Wishlist::distinct('user_id')->count('user_id');
                            ?>
                            <?php echo e($uniqueUsers); ?> users
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-purple-50 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        ?>
                        <?php if($topArtwork): ?>
                            <p class="text-2xl font-bold text-purple-900"><?php echo e($topArtwork->count); ?> saves</p>
...
<section class="py-6 bg-gray-50 border-b">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <form method="GET" class="flex flex-wrap gap-3 items-center">
            ...
            <a href="<?php echo e(route('admin.wishlists.index')); ?>" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition">
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $wishlists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wishlist): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 rounded-full bg-blue-100 border-2 border-white shadow-sm flex items-center justify-center text-blue-700 font-bold overflow-hidden">
                                            <?php if($wishlist->user->avatar): ?>
                                                <img src="<?php echo e(asset('storage/' . $wishlist->user->avatar)); ?>" class="h-full w-full object-cover">
                                            <?php else: ?>
                                                <?php echo e(substr($wishlist->user->name, 0, 1)); ?>

                                            <?php endif; ?>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900"><?php echo e($wishlist->user->name); ?></div>
                                            <div class="text-sm text-gray-500"><?php echo e($wishlist->user->email); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <?php if($wishlist->artwork && $wishlist->artwork->images && is_array($wishlist->artwork->images) && count($wishlist->artwork->images) > 0): ?>
                                            <img src="<?php echo e(asset('storage/' . $wishlist->artwork->images[0])); ?>" class="h-12 w-12 rounded object-cover mr-3" alt="<?php echo e($wishlist->artwork->title); ?>">
                                        <?php else: ?>
                                        ...
                                        <?php endif; ?>
                                        <div>
                                            <div class="text-sm font-medium text-gray-900"><?php echo e($wishlist->artwork?->title ?? 'Deleted Artwork'); ?></div>
                                            <div class="text-sm text-gray-500"><?php echo e($wishlist->artwork?->category?->name ?? 'N/A'); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900"><?php echo e($wishlist->artwork?->artist?->name ?? 'Unknown'); ?></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">
                                        <?php if($wishlist->artwork): ?>
                                            $<?php echo e(number_format($wishlist->artwork->price, 2)); ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php echo e($wishlist->created_at->format('M j, Y')); ?>

                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <?php if($wishlist->artwork): ?>
                                            <a href="<?php echo e(route('public.artworks.show', $wishlist->artwork->slug)); ?>" target="_blank" class="text-blue-600 hover:text-blue-900" title="View Artwork">
                                                <i class="fas fa-external-link-alt"></i>
                                            </a>
                                        <?php endif; ?>
                                        <a href="<?php echo e(route('admin.users.show', $wishlist->user->id)); ?>" class="text-indigo-600 hover:text-indigo-900" title="View User">
                                            <i class="fas fa-user"></i>
                                        </a>
                                        <form method="POST" action="<?php echo e(route('admin.wishlists.delete', $wishlist->id)); ?>" class="inline">
                                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\panchigallery.com\resources\views\admin\wishlists\index.blade.php ENDPATH**/ ?>